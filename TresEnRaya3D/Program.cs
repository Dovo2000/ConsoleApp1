...
namespace TresEnRaya3D
{
    internal class Program
    {
        ...
        static char[,,] tableInfo = new char[MaxRows, MaxColumns, MaxDeep];

        struct Vector3i
        {
            public int X, Y, Z;

            public Vector3i(int posX, int posY, int posZ)
            {
                X = posX;
                Y = posY;
                Z = posZ;
        ...
        static void InitTable()
        {
            ...
            if (parseSucceed)
            {
                ...
                tableInfo = new char[MaxRows, MaxColumns, MaxDeep];
            }
        ...
        static bool StartTurn(int playerNum)
        {
            while (true)    // Turno
            {
                ...
                if (MoveIsValid(pos))
                {
                    tableInfo[pos.X, pos.Y, pos.Z] = playerSymbol;
        ...
        static Vector3i GetPosition(int playerNum, char playerSymbol)
        {
            while (true)
            {
                Console.WriteLine($"Jugador {playerNum} ({playerSymbol}) introduce la fila, la columna y la profundidad (fila,columna,profundidad) en la que quieres introducir tu ficha: ");
                ...
                {
                    Vector3i pos = new Vector3i();
                    bool parseSucceedX = int.TryParse(splittedInput[0], out pos.X);
                    bool parseSucceedY = int.TryParse(splittedInput[1], out pos.Y);
                    bool parseSucceedZ = int.TryParse(splittedInput[2], out pos.Z);

                    if (parseSucceedX && parseSucceedY && parseSucceedZ)
                        if (pos.X > 0 && pos.X <= MaxRows && pos.Y > 0 && pos.Y <= MaxColumns && pos.Z > 0 && pos.Z <= MaxDeep)
                            return pos;
                }
            }
        }

        static bool MoveIsValid(Vector3i pos)
        {
            if (tableInfo[pos.X, pos.Y, pos.Z] != ' ')
            {
